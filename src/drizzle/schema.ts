import { pgTable, foreignKey, unique, pgPolicy, check, uuid, text, boolean, timestamp, integer, date, jsonb, index, bigint, inet, numeric } from "drizzle-orm/pg-core"
...
export const profiles = pgTable("profiles", {
	...
	pgPolicy("Users can insert their own profile", { as: "permissive", for: "insert", to: ["public"], withCheck: sql`(auth.uid() = user_id)`  }),
	...
	check("profiles_approval_status_check", sql`approval_status = ANY (ARRAY['pending'::text, 'approved'::text, 'rejected'::text])`),
	check("profiles_user_type_check", sql`user_type = ANY (ARRAY['agent'::text, 'solicitor'::text, 'conveyancer'::text, 'admin'::text])`),
]);
...
export const userGroups = pgTable("user_groups", {
	...
	pgPolicy("Admins can manage all groups", { as: "permissive", for: "all", to: ["public"], using: sql`(EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.user_id = auth.uid()) AND (profiles.user_type = 'admin'::text))))` }),
	pgPolicy("Users can view groups they belong to", { as: "permissive", for: "select", to: ["public"] }),
	check("user_groups_type_check", sql`type = ANY (ARRAY['agency'::text, 'law_firm'::text, 'conveyancer'::text])`),
]);
...
export const userGroupMembers = pgTable("user_group_members", {
	...
	unique("user_group_members_user_id_group_id_key").on(table.userId, table.groupId),
	pgPolicy("Admins can manage group memberships", { as: "permissive", for: "all", to: ["public"], using: sql`(EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.user_id = auth.uid()) AND (profiles.user_type = 'admin'::text))))` }),
	pgPolicy("Users can view their own group memberships", { as: "permissive", for: "select", to: ["public"] }),
	check("user_group_members_role_check", sql`role = ANY (ARRAY['admin'::text, 'member'::text])`),
]);
...
export const usageReports = pgTable("usage_reports", {
	...
	reportDate: date("report_date").default(sql`CURRENT_DATE`).notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	foreignKey({
			columns: [table.groupId],
	...
	unique("usage_reports_group_id_user_id_api_endpoint_report_date_key").on(table.groupId, table.userId, table.apiEndpoint, table.reportDate),
	pgPolicy("Admins can manage all usage reports", { as: "permissive", for: "all", to: ["public"], using: sql`(EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.user_id = auth.uid()) AND (profiles.user_type = 'admin'::text))))` }),
	pgPolicy("Users can view their own usage reports", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const dataIntegrations = pgTable("data_integrations", {
	...
	pgPolicy("All authenticated users can view integrations", { as: "permissive", for: "select", to: ["authenticated"], using: sql`true` }),
	pgPolicy("Only admins can manage integrations", { as: "permissive", for: "all", to: ["public"] }),
	check("data_integrations_api_type_check", sql`api_type = ANY (ARRAY['google_maps'::text, 'corelogic'::text, 'qld_globe'::text, 'docusign'::text, 'council_api'::text])`),
]);
...
export const auditLogs = pgTable("audit_logs", {
	...
	pgPolicy("Admins can view all audit logs", { as: "permissive", for: "select", to: ["public"], using: sql`(EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.user_id = auth.uid()) AND (profiles.user_type = 'admin'::text))))` }),
	pgPolicy("All authenticated users can insert audit logs", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can view their own audit logs", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const apiUsageLogs = pgTable("api_usage_logs", {
	...
	index("idx_api_usage_logs_created_at").using("btree", table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
	index("idx_api_usage_logs_user_id").using("btree", table.userId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.disclosureFormId],
			foreignColumns: [disclosureForms.id],
			name: "api_usage_logs_disclosure_form_id_fkey"
		}).onDelete("set null"),
	pgPolicy("Admins can view all API usage logs", { as: "permissive", for: "select", to: ["public"], using: sql`(EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.user_id = auth.uid()) AND (profiles.user_type = 'admin'::text))))` }),
	pgPolicy("Users can view their own API usage logs", { as: "permissive", for: "select", to: ["public"] }),
]);
